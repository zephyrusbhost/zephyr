...
#define  REG_GAHBCFG_GINTMSK                       BIT(0)   /* Global interrupt mask.                               */

#define  REG_GAHBCFG_TXFELVL                       BIT(7)   /* Non-periodic TxFIFO empty level.                     */
#define  REG_GAHBCFG_PTXFELVL                      BIT(8)   /* Periodic TxFIFO empty level.                         */

#define  REG_GINTx_CMOD                            BIT(0)   /* Current mode of operation interrupt.                 */
#define  REG_GINTx_OTGINT                          BIT(2)   /* OTG interrupt.                                       */
#define  REG_GINTx_SOF                             BIT(3)   /* Start of frame interrupt.                            */
#define  REG_GINTx_RXFLVL                          BIT(4)   /* RxFIFO non-empty interrupt.                          */
#define  REG_GINTx_NPTXFE                          BIT(5)   /* Non-periodic TxFIFO empty interrupt.                 */
#define  REG_GINTx_IPXFR                           BIT(21)   /* Incomplete periodic transfer interrupt.              */
#define  REG_GINTx_HPRTINT                         BIT(24)   /* Host port interrupt.                                 */
#define  REG_GINTx_HCINT                           BIT(25)   /* Host Channel interrupt.                              */
#define  REG_GINTx_PTXFE                           BIT(26)   /* Periodic TxFIFO empty interrupt.                     */
#define  REG_GINTx_CIDSCHGM                        BIT(28)   /* Connector ID status change mask.                     */
#define  REG_GINTx_DISCINT                         BIT(29)   /* Disconnect detected interrupt.                       */
#define  REG_GINTx_SRQINT                          BIT(30)   /* Session request/new session detected interrupt mask. */
#define  REG_GINTx_WKUPINT                         BIT(31)   /* Resume/remote wakeup detected interrupt mask.        */

#define  REG_GRSTCTL_CORE_SW_RST                   BIT(0)   /* Core soft reset.                                     */
#define  REG_GRSTCTL_HCLK_SW_RST                   BIT(1)   /* HCLK soft reset.                                     */
#define  REG_GRSTCTL_RXFFLUSH                      BIT(4)   /* RxFIFO flush.                                        */
#define  REG_GRSTCTL_TXFFLUSH                      BIT(5)   /* TxFIFO flush.                                        */
#define  REG_GRSTCTL_AHBIDL                        BIT(31)   /* AHB master idle.                                     */
...
#define  REG_GCCFG_PWRDWN                          BIT(16)   /* Power down.                                          */
#define  REG_GCCFG_VBUSASEN                        BIT(18)   /* Enable the VBUS sensing A Device.                    */
#define  REG_GCCFG_VBUSBSEN                        BIT(19)   /* Enable the VBUS sensing B Device.                    */
#define  REG_GCCFG_SOFOUTEN                        BIT(20)   /* SOF output enable.                                   */
#define  REG_GCCFG_VBDEN                           BIT(21)   /* This bit is use for STM32F74xx & STM32F75xx driver.  */
#define  REG_GCCFG_NONVBUSSENS                     BIT(21)   /* This bit is use for STM32F107, STM32F2 & STM32F4.    */

#define  REG_GUSBCFG_PHYSEL                        BIT(6)   /* Full speed serial transceiver select.                */
#define  REG_GUSBCFG_SRPCAP                        BIT(8)   /* SRP-capable.                                         */
#define  REG_GUSBCFG_HNPCAP                        BIT(9)   /* HNP-capable.                                         */
#define  REG_GUSBCFG_FORCE_HOST                    BIT(29)   /* Force host mode.                                     */
...
#define  REG_HCFG_FS_LS_SUPPORT                    BIT(2)   /* FS- and LS-only support.                             */
#define  REG_HCFG_FS_LS_CLK_SEL                   (BIT(1) | BIT(0))

#define  REG_HPTXSTS_REQTYPE                      (BIT(26) | BIT(25))
#define  REG_HPTXSTS_CH_EP_NBR                    (BIT(30) | BIT(29) | BIT(28) | BIT(27))
#define  REG_HPTXSTS_REQTYPE_IN_OUT                0u
#define  REG_HPTXSTS_REQTYPE_ZLP                   1u
#define  REG_HPTXSTS_REQTYPE_DIS_CH_CMD            3u

#define  REG_HCCHARx_EPDIR                         BIT(15)   /* Endpoint direction.                                  */
#define  REG_HCCHARx_LOW_SPEED_DEV                 BIT(17)   /* Low speed device.                                    */
#define  REG_HCCHARx_MC_EC                        (BIT(21) | BIT(20))
#define  REG_HCCHARx_ODDFRM                        BIT(29)   /* Odd frame.                                           */
#define  REG_HCCHARx_CHDIS                         BIT(30)   /* Channel disable.                                     */
#define  REG_HCCHARx_CHENA                         BIT(31)   /* Channel enable.                                      */
...
#define  REG_HCINTx_XFRC                           BIT(0)   /* transfer complete interrupt.                         */
#define  REG_HCINTx_CH_HALTED                      BIT(1)   /* Channel halted interrupt.                            */
#define  REG_HCINTx_STALL                          BIT(3)   /* STALL response received interrupt.                   */
#define  REG_HCINTx_NAK                            BIT(4)   /* NAK response received interrupt.                     */
#define  REG_HCINTx_ACK                            BIT(5)   /* ACK response received/transmitted interrupt.         */
#define  REG_HCINTx_TXERR                          BIT(7)   /* Transaction error interrupt.                         */
#define  REG_HCINTx_BBERR                          BIT(8)   /* Babble error interrupt.                              */
#define  REG_HCINTx_DTERR                          BIT(10)   /* Data toggle error interrupt.                         */
...
#define  REG_HPRT_PORT_CONN_STS                    BIT(0)   /* Port connect status.                                 */
#define  REG_HPRT_PORT_CONN_DET                    BIT(1)   /* Port connect detected.                               */
#define  REG_HPRT_PORT_EN                          BIT(2)   /* Port enable.                                         */
#define  REG_HPRT_PORT_EN_CHNG                     BIT(3)   /* Port enable/disable change.                          */
#define  REG_HPRT_PORT_OCCHNG                      BIT(5)   /* Port overcurrent change.                             */
#define  REG_HPRT_PORT_SUSP                        BIT(7)   /* Port suspend.                                        */
#define  REG_HPRT_PORT_RST                         BIT(8)   /* Port reset.                                          */
#define  REG_HPRT_PORT_PWR                         BIT(12)   /* Port power.                                          */
#define  REG_HPRT_PORT_SPEED                      (BIT(17) | BIT(18))
...
typedef  struct  usbh_stm32fx_host_ch_reg {                     /* ------------- HOST CHANNEL-X DATA TYPE ------------- */
    volatile uint32_t                 HCCHARx;                          /* Host channel-x characteristics                       */
    volatile uint32_t                 HCSPLTx;                          /* Host channel-x split control                         */
    volatile uint32_t                 HCINTx;                           /* Host channel-x interrupt                             */
    volatile uint32_t                 HCINTMSKx;                        /* Host channel-x interrupt mask                        */
    volatile uint32_t                 HCTSIZx;                          /* Host channel-x transfer size                         */
    volatile uint32_t                 HCDMAx;                           /* Host channel-x DMA address                           */
    volatile uint32_t                 RSVD1[2u];
} USBH_STM32FX_HOST_CH_REG;


typedef  struct  usbh_stm32fx_dfifo_reg {                       /* ---------- DATA FIFO ACCESS DATA TYPE -------------- */
    volatile uint32_t                 DATA[STM32FX_DFIFO_SIZE];         /* 4K bytes per endpoint                                */
} USBH_STM32FX_DFIFO_REG;


typedef  struct  usbh_stm32fx_reg {
                                                                /* ----- CORE GLOBAL CONTROL AND STATUS REGISTERS ----- */
    volatile uint32_t                 GOTGCTL;                          /* 0x0000 Core control and status                       */
    volatile uint32_t                 GOTGINT;                          /* 0x0004 Core interrupt                                */
    volatile uint32_t                 GAHBCFG;                          /* 0x0008 Core AHB configuration                        */
    volatile uint32_t                 GUSBCFG;                          /* 0x000C Core USB configuration                        */
    volatile uint32_t                 GRSTCTL;                          /* 0x0010 Core reset                                    */
    volatile uint32_t                 GINTSTS;                          /* 0x0014 Core interrupt                                */
    volatile uint32_t                 GINTMSK;                          /* 0x0018 Core interrupt mask                           */
    volatile uint32_t                 GRXSTSR;                          /* 0x001C Core receive status debug read                */
    volatile uint32_t                 GRXSTSP;                          /* 0x0020 Core status read and pop                      */
    volatile uint32_t                 GRXFSIZ;                          /* 0x0024 Core receive FIFO size                        */
    volatile uint32_t                 HNPTXFSIZ;                        /* 0x0028 Host non-periodic transmit FIFO size          */
    volatile uint32_t                 HNPTXSTS;                         /* 0x002C Core Non Periodic Tx FIFO/Queue status        */
    volatile uint32_t                 RSVD0[2u];
    volatile uint32_t                 GCCFG;                            /* 0x0038 General core configuration                    */
    volatile uint32_t                 CID;                              /* 0x003C Core ID register                              */
    volatile uint32_t                 RSVD1[48u];
    volatile uint32_t                 HPTXFSIZ;                         /* 0x0100 Core Host Periodic Tx FIFO size               */
    volatile uint32_t                 RSVD2[191u];
                                                                /* ------ HOST MODE CONTROL AND STATUS REGISTERS ------ */
    volatile uint32_t                 HCFG;                             /* 0x0400 Host configuration                            */
    volatile uint32_t                 HFIR;                             /* 0x0404 Host frame interval                           */
    volatile uint32_t                 HFNUM;                            /* 0x0408 Host frame number/frame time remaining        */
    volatile uint32_t                 RSVD3;
    volatile uint32_t                 HPTXSTS;                          /* 0x0410 Host periodic transmit FIFO/queue status      */
    volatile uint32_t                 HAINT;                            /* 0x0414 Host all channels interrupt                   */
    volatile uint32_t                 HAINTMSK;                         /* 0x0418 Host all channels interrupt mask              */
    volatile uint32_t                 RSVD4[9u];
    volatile uint32_t                 HPRT;                             /* 0x0440 Host port control and status                  */
    volatile uint32_t                 RSVD5[47u];
    USBH_STM32FX_HOST_CH_REG  HCH[16u];                         /* 0x0500 Host Channel-x regiters                       */
    volatile uint32_t                 RSVD6[448u];
                                                                /* -- POWER & CLOCK GATING CONTROL & STATUS REGISTER -- */
    volatile uint32_t                 PCGCR;                            /* 0x0E00 Power anc clock gating control                */
    volatile uint32_t                 RSVD7[127u];
...
typedef  struct  usbh_stm32fx_ch_info {
    uint16_t            EP_Addr;                              /* Device addr | EP DIR | EP NBR.                       */
    uint32_t            EP_PktCnt;                            /* For OUT EP packet count.                             */
    uint8_t            DataTgl;
    uint32_t            CurDataTgl;                           /* Data tggl for retransmit (no-ACK resp from device)   */
    uint8_t            CurXferErrCnt;                        /* For IN and OUT xfer when Transaction Error intr      */
    uint8_t            HaltSrc;                              /* Indicates source of the halt interrupt.              */
    bool           Halting;                              /* Flag indicating channel halt in progress.            */
    bool           LastTransaction;                      /* For IN processing with Rx FIFO Non-Empty intr        */
    bool           Aborted;                              /* Flag indicating channel has been aborted.            */
                                                                /* To ensure the URB EP xfer size is not corrupted ...  */
    uint32_t            AppBufLen;                            /* ... for multi-transaction transfer                   */
    uint8_t           *AppBufPtr;                            /* Ptr to buf supplied by app.                          */
    USBH_URB             *URB_Ptr;
} USBH_STM32FX_CH_INFO;


typedef  struct  usbh_drv_data {
    uint8_t            RH_Desc[USBH_HUB_LEN_HUB_DESC];       /* RH desc content.                                     */
    uint8_t            ChMaxNbr;                             /* Max number of host channels.                         */
    USBH_STM32FX_CH_INFO  ChInfoTbl[STM32FX_OTG_FS_MAX_NBR_CH]; /* Contains information of host channels.               */
    uint16_t            ChUsed;                               /* Bit array for BULK, ISOC, INTR, CTRL channel mgmt.   */
    uint16_t            RH_PortConnStatChng;                  /* Port connection status change for RH.                */
    uint16_t            RH_PortResetChng;                     /* Port reset change for RH.                            */
    bool           RH_Init;
} USBH_DRV_DATA;
...
static  uint32_t    USBH_STM32FX_HCD_FrameNbrGet     (USBH_HC_DRV           *p_hc_drv,
                                                        USBH_ERR              *p_err);
...
static  bool   USBH_STM32FX_HCD_IsHalt_EP       (USBH_HC_DRV           *p_hc_drv,
                                                        USBH_EP               *p_ep,
                                                        USBH_ERR              *p_err);
...
static  bool   USBH_STM32FX_HCD_PortStatusGet   (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr,
                                                        USBH_HUB_PORT_STATUS  *p_port_status);

static  bool   USBH_STM32FX_HCD_HubDescGet      (USBH_HC_DRV           *p_hc_drv,
                                                        void                  *p_buf,
                                                        uint8_t             buf_len);

static  bool   USBH_STM32FX_HCD_PortEnSet       (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortEnClr       (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortEnChngClr   (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortPwrSet      (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortPwrClr      (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortResetSet    (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortResetChngClr(USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortSuspendClr  (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_PortConnChngClr (USBH_HC_DRV           *p_hc_drv,
                                                        uint8_t             port_nbr);

static  bool   USBH_STM32FX_HCD_RHSC_IntEn      (USBH_HC_DRV           *p_hc_drv);

static  bool   USBH_STM32FX_HCD_RHSC_IntDis     (USBH_HC_DRV           *p_hc_drv);
...
static  uint8_t  STM32FX_GetHostChNbr      (USBH_DRV_DATA     *p_drv_data,
                                               uint16_t         ep_addr);

static  uint8_t  STM32FX_GetFreeHostChNbr  (USBH_DRV_DATA     *p_drv_data,
                                               USBH_ERR          *p_err);
                                             ...
                                             USBH_ERR     *p_err)
{
    ...
    p_drv_data->RH_PortConnStatChng = 0x00;
    p_drv_data->RH_PortResetChng    = 0x00;
                                         ...
                                         USBH_ERR     *p_err)
{
    ...
    if (ep_nbr != 0u) {                                         /* Non-Control EPs.                                     */
        ...
        if (ch_nbr != 0xFFu) {
            ...
            DEF_BIT_CLR(p_drv_data->ChUsed, BIT(ch_nbr));
        }
                                           ...
                                           USBH_ERR     *p_err)
{
    ...
    reg_val  =  0u;
    reg_val  = (uint32_t)(dev_addr << 22u);                   /* Set the device address to which the channel belongs  */
    reg_val |= (uint32_t)(ep_nbr   << 11u);                   /* Set the associated EP address                        */
    ...
    reg_val |= (uint32_t)(ep_type << 18u);                    /* Set the endpoint type                                */
    reg_val |= (uint32_t)ep_max_pkt_size;                     /* Set the maximum endpoint size                        */
                                                                /* Set the Multi Count field (bits 21-20). See Note #3  */
    reg_val |= (uint32_t)(REG_HCCHARx_1_TRANSACTION << 20u) & REG_HCCHARx_MC_EC;
    ...
    switch (ep_type){
        ...
        case USBH_EP_TYPE_INTR:                                 /* ------------------- INTERRUPT CHANNEL -------------- */
             ...
             if (DEF_BIT_IS_CLR(p_reg->HFNUM, BIT(0)) == DEF_YES) {
    ...
    if (p_urb->Err != USBH_ERR_EP_NACK) {                       /* if not URB retransmission, save User buffer length.. */
        ...
        p_drv_data->ChInfoTbl[ch_nbr].AppBufPtr = (uint8_t *)p_urb->UserBufPtr;
    }
                                          ...
                                          USBH_ERR     *p_err)
{
    ...
    uint8_t             ep_nbr;
    uint8_t             ep_dir;
    uint8_t             ch_nbr_ctrl_in;
    uint8_t             ch_nbr_ctrl_out;
    uint8_t             ch_nbr;
    uint16_t             dev_addr;
    uint16_t             retry_cnt;
    bool            ch_halt_status;
                                          ...
                                          USBH_DRV_DATA     *p_drv_data)
{
    ...
    p_data_buf = (uint32_t *)p_ch_info->AppBufPtr;
                                   ...
                                   uint8_t         ep_type)
{
    ...
    reg_val  = ((uint32_t)data_pid) << 29u;                   /* PID. See Note #2                                     */
    reg_val |= ((uint32_t)nbr_pkts) << 19u;                   /* Packet Count                                         */
    ...
    if ((ep_is_in == 0u) && xfer_len > 0u) {                    /* Load OUT packet into the appropriate Tx FIFO.        */
        ...
        if (p_urb->XferLen != 0) {                              /* Keep track of data to be sent                        */
            ...
            p_user_buf = (uint32_t *)p_urb->UserBufPtr + xfer_len_in_words;
        } else {
            p_user_buf = (uint32_t *)p_urb->UserBufPtr;
        }
                             ...
                             uint16_t         bytes)
{
    ...
    volatile uint32_t   *p_fifo;
    uint32_t  *p_data_buff;


    dword_count = (bytes + 3u) / 4u;                            /* See Note #1                                          */

    p_fifo      =  p_reg->DFIFO[ch_nbr].DATA;
    p_data_buff = (uint32_t *)p_src;
