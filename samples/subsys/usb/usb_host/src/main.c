			     ...
			     void *p_ctx)
{
	...
	if (usb_err == 0) {
		...
		if (usbh_msc_init(p_msc_dev, lun) == 0) {
			...
			LOG_INF("unit rdy %d\n", usbh_msc_unit_rdy_test(p_msc_dev, lun, &usb_err));

			int ret = usbh_msc_write(p_msc_dev, lun, 0, 1, blocksize, &buf, &usb_err);

			LOG_INF("written %d bytes\n", ret);

			uint8_t temp[512];

			ret = usbh_msc_read(p_msc_dev, lun, 0, 1, blocksize, &temp, &usb_err);

			LOG_INF("read %d bytes\n", ret);

			for (int i = 0; i < 512; i++) {
				if (temp[i] != buf[i]) {
					LOG_INF("write and read buffers are not equal\n");
					return;
				}
			}
