...
K_MEM_POOL_DEFINE(AsyncURB_PPool, sizeof(USBH_URB), sizeof(USBH_URB), (USBH_CFG_MAX_NBR_DEVS * USBH_CFG_MAX_EXTRA_URB_PER_DEV), sizeof(CPU_ALIGN));
...
static USBH_ERR usbh_ep_open(USBH_DEV *p_dev, USBH_IF *p_if,
							 uint8_t ep_type,
							 uint8_t ep_dir,
							 USBH_EP *p_ep);
...
static void usb_urb_notify(USBH_URB *p_urb);

static USBH_ERR usbh_urb_submit(USBH_URB *p_urb);

static void usbh_urb_clr(USBH_URB *p_urb);

static USBH_ERR usbh_dflt_ep_open(USBH_DEV *p_dev);

static USBH_ERR usbh_dev_desc_rd(USBH_DEV *p_dev);

static USBH_ERR usbh_cfg_rd(USBH_DEV *p_dev, uint8_t cfg_ix);

static USBH_ERR usbh_cfg_parse(USBH_DEV *p_dev, USBH_CFG *p_cfg);

static USBH_ERR usbh_dev_addr_set(USBH_DEV *p_dev);

static uint32_t usbh_str_desc_get(USBH_DEV *p_dev, uint8_t desc_ix,
								  uint16_t lang_id,
								  void *p_buf,
								  uint32_t buf_len,
								  USBH_ERR *p_err);
...
static USBH_DESC_HDR *usbh_next_desc_get(void *p_buf, uint32_t *p_offset);

static void usbh_fmt_setup_req(USBH_SETUP_REQ *p_setup_req, void *p_buf_dest);

static void usbh_parse_dev_desc(USBH_DEV_DESC *p_dev_desc, void *p_buf_src);

static void usbh_parse_cfg_desc(USBH_CFG_DESC *p_cfg_desc, void *p_buf_src);

static void usbh_parse_if_desc(USBH_IF_DESC *p_if_desc, void *p_buf_src);

static void usbh_parse_ep_desc(USBH_EP_DESC *p_ep_desc, void *p_buf_src);
				   ...
				   USBH_KERNEL_TASK_INFO *hub_task_info)
{
	...
	k_thread_create(&USBH_Host.HAsyncTask, USBH_AsyncTask_Stack,
					...
					usbh_async_task, NULL, NULL, NULL,
					0, 0, K_NO_WAIT);
...
USBH_ERR usbh_hc_port_en(uint8_t hc_nbr, uint8_t port_nbr)
{
	LOG_DBG("PortEn");
	USBH_ERR err;
	USBH_HC *p_hc;
...
USBH_ERR usbh_hc_port_dis(uint8_t hc_nbr, uint8_t port_nbr)
{
	LOG_DBG("PortDis");
	USBH_ERR err;
	USBH_HC *p_hc;
...
uint32_t usbh_hc_frame_nbr_get(uint8_t hc_nbr, USBH_ERR *p_err)
{
	uint32_t frame_nbr;
	USBH_HC *p_hc;
...
void usbh_dev_desc_get(USBH_DEV *p_dev, USBH_DEV_DESC *p_dev_desc)
{
...
USBH_ERR usbh_cfg_set(USBH_DEV *p_dev, uint8_t cfg_nbr)
{
	USBH_ERR err;
	USBH_SET_CFG(
		p_dev, cfg_nbr,
...
USBH_CFG *usbh_cfg_get(USBH_DEV *p_dev, uint8_t cfg_ix)
{
	uint8_t nbr_cfgs;
	...
	if ((cfg_ix >= nbr_cfgs) || (nbr_cfgs == 0u))
	{
		return ((USBH_CFG *)0);
	}
...
USBH_ERR usbh_cfg_desc_get(USBH_CFG *p_cfg, USBH_CFG_DESC *p_cfg_desc)
{
	USBH_DESC_HDR *p_desc;

	if ((p_cfg == (USBH_CFG *)0) || (p_cfg_desc == (USBH_CFG_DESC *)0))
	{
		return (USBH_ERR_INVALID_ARG);
	}
...
USBH_DESC_HDR *usbh_cfg_extra_desc_get(USBH_CFG *p_cfg, USBH_ERR *p_err)
{
	USBH_DESC_HDR *p_desc;
...
USBH_IF *usbu_if_get(USBH_CFG *p_cfg, uint8_t if_ix)
{
	uint8_t nbr_ifs;
...
uint8_t usbh_if_ep_nbr_get(USBH_IF *p_if, uint8_t alt_ix)
{
	USBH_DESC_HDR *p_desc;
	uint32_t if_off;
...
USBH_ERR usbh_if_desc_get(USBH_IF *p_if, uint8_t alt_ix,
...
uint8_t *usbh_if_extra_desc_get(USBH_IF *p_if, uint8_t alt_ix,
								 uint16_t *p_data_len)
{
	USBH_DESC_HDR *p_desc;
	uint8_t *p_data;
...
USBH_ERR usbh_bulk_in_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
USBH_ERR usbh_bulk_out_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
USBH_ERR usbh_intr_in_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
USBH_ERR usbh_intr_out_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
USBH_ERR usbh_isoc_in_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
USBH_ERR usbh_isoc_out_open(USBH_DEV *p_dev, USBH_IF *p_if, USBH_EP *p_ep)
{
	USBH_ERR err;
...
uint16_t usbh_ctrl_tx(USBH_DEV *p_dev, uint8_t b_req,
					   uint8_t bm_req_type,
					   uint16_t w_val,
					   ...
					   USBH_ERR *p_err)
{
	...
	{
		xfer_len = usbh_sync_ctrl_transfer(&p_dev->DfltEP, b_req, bm_req_type,
...
uint16_t usbh_ctrl_rx(USBH_DEV *p_dev, uint8_t b_req,
					   uint8_t bm_req_type,
					   uint16_t w_val,
					   uint16_t w_ix, void *p_data,
					   uint16_t w_len,
...
uint32_t usbh_bulk_tx(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
					   uint32_t timeout_ms,
					   USBH_ERR *p_err)
{
	uint8_t ep_type;
	...
	xfer_len = usbh_sync_transfer(p_ep, p_buf, buf_len, (USBH_ISOC_DESC *)0,
...
USBH_ERR usbh_bulk_tx_async(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
						  USBH_XFER_CMPL_FNCT fnct, void *p_fnct_arg)
{
	...
	err = usbh_async_transfer(p_ep, p_buf, buf_len, (USBH_ISOC_DESC *)0,
						 USBH_TOKEN_OUT, (void *)fnct, p_fnct_arg);
...
uint32_t usbh_bulk_rx(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
					   uint32_t timeout_ms,
					   USBH_ERR *p_err)
{
	uint8_t ep_type;
...
USBH_ERR usbh_bulk_rx_async(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
						  USBH_XFER_CMPL_FNCT fnct, void *p_fnct_arg)
{
	USBH_ERR err;
	uint8_t ep_type;
...
uint32_t usbh_intr_tx(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
					   uint32_t timeout_ms,
					   USBH_ERR *p_err)
{
	uint8_t ep_type;
...
USBH_ERR usbh_intr_tx_async(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
						  USBH_XFER_CMPL_FNCT fnct, void *p_fnct_arg)
{
	USBH_ERR err;
	uint8_t ep_type;
...
uint32_t usbh_intr_rx(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
					   uint32_t timeout_ms,
					   USBH_ERR *p_err)
{
	uint8_t ep_type;
...
USBH_ERR usbh_intr_rx_async(USBH_EP *p_ep, void *p_buf, uint32_t buf_len,
						  USBH_XFER_CMPL_FNCT fnct, void *p_fnct_arg)
{
	USBH_ERR err;
	uint8_t ep_type;
...
uint32_t usbh_isoc_tx(USBH_EP *p_ep, uint8_t *p_buf, uint32_t buf_len,
					   uint32_t start_frm,
					   uint32_t nbr_frm,
					   uint16_t *p_frm_len,
					   USBH_ERR *p_frm_err,
...
USBH_ERR usbh_isoc_tx_async(USBH_EP *p_ep, uint8_t *p_buf, uint32_t buf_len,
						  uint32_t start_frm,
						  uint32_t nbr_frm,
						  uint16_t *p_frm_len,
						  USBH_ERR *p_frm_err,
						  USBH_ISOC_CMPL_FNCT fnct, void *p_fnct_arg)
{
	...
	err = usbh_async_transfer(p_ep, p_buf, buf_len, p_isoc_desc, USBH_TOKEN_IN,
...
uint32_t usbh_isoc_rx(USBH_EP *p_ep, uint8_t *p_buf, uint32_t buf_len,
					   uint32_t start_frm,
					   uint32_t nbr_frm,
					   uint16_t *p_frm_len,
					   USBH_ERR *p_frm_err,
...
USBH_ERR usbh_isoc_rx_async(USBH_EP *p_ep, uint8_t *p_buf, uint32_t buf_len,
						  uint32_t start_frm,
						  uint32_t nbr_frm,
						  uint16_t *p_frm_len,
						  USBH_ERR *p_frm_err,
...
USBH_ERR usbh_ep_stall_set(USBH_EP *p_ep)
{
	...
	(void)usbh_ctrl_tx(p_dev, USBH_REQ_SET_FEATURE,
					  ...
					  (void *)0, 0u, USBH_CFG_STD_REQ_TIMEOUT, &err);
	if (err != USBH_ERR_NONE)
	{
...
USBH_ERR usbh_ep_reset(USBH_DEV *p_dev, USBH_EP *p_ep)
{
	USBH_EP *p_ep_t;
	USBH_ERR err;
...
uint32_t usbh_str_get(USBH_DEV *p_dev, uint8_t desc_ix, uint16_t lang_id,
					   ...
					   USBH_ERR *p_err)
{
	...
	{ /* If lang ID is zero, get dflt used by the dev.        */
		str_len = usbh_str_desc_get(p_dev, 0u, 0u, p_buf, buf_len, p_err);
		if (str_len <
			4u)
		{ /* See Note #1.                                         */
									...
									USBH_ERR *p_err)
{
	...
	len = usbh_sync_transfer(p_ep, (void *)&setup_buf[0u], USBH_LEN_SETUP_PKT,
						(USBH_ISOC_DESC *)0, USBH_TOKEN_SETUP, timeout_ms,
						p_err);
	if (*p_err != USBH_ERR_NONE)
	{
		return (0u);
	...
	{ /* -------------------- DATA STAGE -------------------- */
		...
		rtn_len =
			usbh_sync_transfer(p_ep, (void *)p_data_08, w_len,
						  (USBH_ISOC_DESC *)0,
...
static USBH_ERR usbh_dev_desc_rd(USBH_DEV *p_dev)
{
	...
	while (retry > 0u)
	{
		...
		USBH_GET_DESC(
			p_dev, USBH_DESC_TYPE_DEV, 0u, p_dev->DevDesc,
			8u, /* See Note (1).                                        */
			&err);
		if (err != USBH_ERR_NONE)
		{
	...
	while (retry > 0u)
	{
		retry--;
		/* ---------------- RD FULL DEV DESC. ----------------- */
		USBH_GET_DESC(p_dev, USBH_DESC_TYPE_DEV, 0u, p_dev->DevDesc,
...
static USBH_ERR usbh_cfg_parse(USBH_DEV *p_dev, USBH_CFG *p_cfg)
{
	...
	if ((cfg_desc.bMaxPower > 250u) || (cfg_desc.bNbrInterfaces == 0u))
	{
		return (USBH_ERR_DESC_INVALID);
	}
								  ...
								  USBH_ERR *p_err)
{
	...
	for (i = 0u; i < USBH_CFG_STD_REQ_RETRY;
		 i++)
	{ /* Retry up to 3 times.                                 */
		len = usbh_ctrl_rx(
		...
		if ((len == 0u) || (*p_err == USBH_ERR_EP_STALL))
		{
			usbh_ep_reset(
				p_dev,
				(USBH_EP *)0); /* Rst EP to clr HC halt state.                         */
		}
		else
		{
			break;
